jQuery("input").each(
  function(index, value) {
    ...
    if ((prefix == "Classes.CalendarEventClass")
        && ((suffix == "eventDate") || (suffix == "eventDate_end"))) {
        ...
        .blur(
            function() {
              ...
              if ((endDate < startDate) && ((endDate != "") && (startDate != ""))) {
                ...
                errorMesgDialog.setHeader(Validation.messages.get(
                    "validate-warning-header"));
                var validationMessage = Validation.messages.get(
                    "validate-dateRange").replace(/<FROM_DATE>/g, startDateStr).replace(
                    /<TO_DATE>/g, endDateStr);
